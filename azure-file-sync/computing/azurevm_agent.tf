resource "azurerm_virtual_machine" "agent" {
  ...
  name                             = "filesync-agent-${var.suffix}"
...
resource "azurerm_virtual_machine_data_disk_attachment" "data" {
  ...
  virtual_machine_id = azurerm_virtual_machine.agent.id
  lun                = "10"
  caching            = "ReadWrite"
}
