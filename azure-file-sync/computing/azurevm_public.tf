...
resource "azurerm_virtual_machine" "public" {
  depends_on                       = [azurerm_network_interface_security_group_association.public]
  ...
  storage_os_disk {
    name              = "FileSyncPublic${var.suffix}"
    caching           = "ReadWrite"
...
# ----------------------------------------------------------------------------------------------
resource "azurerm_managed_disk" "public" {
  name                 = "filesync-public-disk"
  location             = var.resource_group_location
  resource_group_name  = var.resource_group_name
...
# ----------------------------------------------------------------------------------------------
resource "azurerm_virtual_machine_data_disk_attachment" "public" {
  managed_disk_id    = azurerm_managed_disk.public.id
